...
module.exports = (toolbox: GluegunToolbox) => {
  ...
  async function start() {
    ...
    if (!projectConfig) {
      spinner.stop()
      print.info(
        `${print.xmark} Coudn´t find ${config.project.configFileName} file...`
      )
      return
    }

    const designerHistoryFolderName = JSON.parse(projectConfig).architecture
      .designer
    ...
    try {
      ...
      const jsExists = await filesystem.existsAsync(
        filesystem.path(designerTempServerPath, designerJsFileName)
      )

      const cssExists = await filesystem.existsAsync(
        filesystem.path(designerTempServerPath, designerCssFileName)
      )

      if (jsExists && cssExists) {
        print.info(
          'Coudn´t sync Coffee Designer server files, switching to temporary cached files.'
        )
        print.info('Started Coffee Designer...')
  ...
  async function hasChangeClassTables(): Promise<boolean> {
    const latestVersion = await toolbox.localStorage.getValue(
      'lastCoffeeVersion'
    )
  ...
  async function getClassTables(): Promise<Array<ClassTable>> {
    ...
    const designerVersionFolderName = JSON.parse(projectConfig).architecture
      .designer

    const jsonData = await filesystem.readAsync(
      filesystem.path(designerVersionFolderName, currentVersion + '.json')
    )
    ...
    const currentClassTables = JSON.parse(jsonData).data.classTables as Array<
      ClassTable
    >
    ...
    for (let index = 0; index < currentClassTables.length; index++) {
      ...
      const countForeignKeys = _.sumBy(
        classTable.properties,
        property => property.isForeignKey
      )
      currentClassTables[index].isMiddleTable =
        countForeignKeys === 2 ? true : false
    }

    // Check for associations based on middle tables
    const middleTables = currentClassTables.filter(
      classTable => classTable.isMiddleTable
    )
          ...
          if (!classTable.isMiddleTable) {
            ...
            if (foreignProperty) {
              ...
              if (!currentClassTables[index].tableRelations) {
                currentClassTables[index].tableRelations = new Array<
                  ClassTable
                >()
              }
              ...
              const alreadyHasRelation = currentClassTables[
                index
              ].tableRelations.find(tr => tr.name === middleTable.name)
  ...
  async function syncFiles(
    ...
    // Download JS
    const designerJsFileResponse = await baseRepositoryDownloadFile.get(
      `dist/${designerJsFileName}`
    )
    ...
    await _validateResponse(designerJsFileResponse, async () => {
      await filesystem.writeAsync(
        ...
        designerJsFileResponse.data as any
      )
    })

    // Download CSS
    const designerCssFileResponse = await baseRepositoryDownloadFile.get(
      `dist/${designerCssFileName}`
    )

    await _validateResponse(designerCssFileResponse, async () => {
      await filesystem.writeAsync(
        filesystem.path(serverPath, designerCssFileName),
        designerCssFileResponse.data as any
      )
    })

    // Read Assets files server dir and download them
    const assetsResponse = await baseRepositoryApiFile.get('dist/assets')
    ...
    await _validateResponse(assetsResponse, async () => {
      ...
      for (
        let index = 0;
        index < (assetsResponse.data as any).length;
        index++
      ) {
  ...
  async function _validateResponse(
    ...
    // Repository might be deleted or the files assets has change the name
      ...
      throw Error(
        'Internert connection failed, make sure you´re connected ' +
  ...
  async function getLatestDesignerFileVersion() {
    ...
    for (let index = 0; index < dirTree.children.length; index++) {
      const file = dirTree.children[index]
