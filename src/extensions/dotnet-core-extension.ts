...
module.exports = (toolbox: GluegunToolbox) => {
  ...
  async function updateMigrations() {
    ...
    try { 
        ...
        let latestVersionChangeVersion = 
            await toolbox.designer.getLatestDesignerFileVersion();
  ...
  async function checkConnectionString() {
    ...
    for (let index = 0; index < autofac.modules.length; index++) {
        ...
        const askForConnectionString = async () => {
            ...
            do {
                ...
                const resultConnection = 
                    await toolbox.database.validateConnection(_connection);
        ...
        else if (!isEmptyObject(mod.properties) && mod.properties.ConnectionString) {
            spinner.stop();
            const connectionString = autofac.modules[index].properties.ConnectionString;

            const arrayParts = connectionString.split(';');

            if(arrayParts.length < 5) {
                print.info(`${print.xmark} invalid connection string on "autofac.json"`);
                connection = await askForConnectionString();
    ...
    const autofacFile = filesystem.path(
        webapiFolderPath, 
        'autofac.json'
    );
