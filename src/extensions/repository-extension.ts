...
module.exports = (toolbox: GluegunToolbox) => {
  ...
  async function create() {
    ...
    const repositoriesInfraEntityFolderPath = filesystem.path(
      infrastructureFolderPath,
      'EntityFrameworkDataAccess/Repositories'
    )
    const repositoriesInfraDapperFolderPath = filesystem.path(
      infrastructureFolderPath,
      'DapperDataAccess/Repositories'
    )
    ...
    const repositoryName = toolbox.utils.capitalize(
    ...
    if (prompResult.repositoryType === 'WriteOnly') {
      ...
      await generate({
        ...
        props: {
          repositoryName,
          classTable: classTables.find(
            ct => ct.name === domainModelResult.model
          ),
    ...
    // ReadOnly
      ...
      await generate({
        template:
          'generator/dotnetCore/repository/ReadOnlyRepositoryQuery.ts.ejs',
        target: filesystem.path(
          repositoriesInfraDapperFolderPath,
