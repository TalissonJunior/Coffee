...
module.exports = (toolbox: GluegunToolbox) => {
  ...
  function replaceByMatch(
    ...
    if (startKeywordMatchIndex > -1) {
      const startKeywordMatchIndexCloseBracket = data
        .substr(startKeywordMatchIndex)
        .indexOf(endKeyword)
      ...
      if (startKeywordMatchIndexCloseBracket > -1) {
        ...
        const endDataWithoutReplacement = data.substr(
          startKeywordMatchIndex + startKeywordMatchIndexCloseBracket
        )

        newData =
          initialDataWithoutReplacement +
          ' ' +
          replacement +
          ' ' +
