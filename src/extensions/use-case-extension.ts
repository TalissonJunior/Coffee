...
module.exports = (toolbox: GluegunToolbox) => {
  ...
  async function create() {
    ...
    const outputsApplicationFolderPath = filesystem.path(applicationFolderPath, 'Outputs');    
    const inputsApplicationFolderPath = filesystem.path(applicationFolderPath, 'Inputs');    
    const useCasesApplicationFolderPath = filesystem.path(applicationFolderPath, 'UseCases');    
    const useCasesWebApiFolderPath = filesystem.path(webapiFolderPath, 'UseCases');    
    const treeRepositories = await filesystem.inspectTreeAsync(repositoriesApplicationFolderPath);
    ...
    else if(!prompResult.selectedRepositories || prompResult.selectedRepositories.length === 0) {
        spinner.fail('You must select at least one repository.');
        return;
    }

    // Format usecasename
    prompResult.usecaseName = toolbox.strings.pascalCase(prompResult.usecaseName
        .replace(/UseCase/g, '')
        .replace(/usecase/g, '')
        .replace(/useCase/g, '')
        .replace(/Usecase/g, ''));
    ...
    if(await filesystem.existsAsync(useCaseFolder)) {
        spinner.fail(`Already exists an usecase "${prompResult.usecaseName}UseCase"`);
        return;
    }
    ...
    const applicationUsecaseInterfaceFilePath = 
        filesystem.path(
            useCaseFolder, 
            `I${prompResult.usecaseName}UseCase.cs`
        );

    const applicationUsecaseFilePath = 
        filesystem.path(
            useCaseFolder, 
    ...
    const webapiUsecasePresenterFilePath = 
        filesystem.path(
            webapiUseCaseFolder, 
            `Presenter.cs`
        );

    const webapiUsecaseControllerFilePath = 
        filesystem.path(
            webapiUseCaseFolder, 
